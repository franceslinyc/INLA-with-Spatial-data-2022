...
Section 2 contains key components that make up INLA. Section 3 introduces INLA. Section 4 includes discussion. Two spatial examples using the `R-INLA` package are included in the appendix. 
...
Inference in hierarchical models is based on the posterior distribution, and the posterior distribution, structured in a hierarchical way, becomes 
$$
...
The Laplace approximation was once a key tool for high-dimensional integration pre-MCMC times.
...
$$
\log g(x) \approx \log g(\hat{x}) + \frac{\partial \log g(\hat{x})} {\partial x} {(x-\hat{x})} + \frac{\partial^2 \log g(\hat{x})} {\partial x^2} \frac{1} {2!} {(x-\hat{x})}^2
$$
$$
 = \log g(\hat{x}) + \frac{\partial^2 \log g(\hat{x})} {2 \partial x^2} {(x-\hat{x})}^2  
$$
$$
...
$$
where $\tilde{p}(\cdot|\cdot)$ is an approximated conditional density of its arguments (Rue et al., 2009). The goal is therefore to approximate $p(\theta|y)$ integrate out $\theta_{-j}$ from $\tilde{p}(\theta_j|y)$ and approximate $p(\theta|y)$ and $p(x_i|\theta,y)$ and use numerical integration to integrate out $\theta$ for $\tilde{p}(x_i | y)$. 
...
Since $|\theta|$ is of low dimension, marginals for $\theta_j|y$ can be derived directly from the approximation to $\theta|y$. This might be costly since each new $\theta$ would require an evaluation of equation $(*)$ 
$$
\tilde{p}{(x|\theta, y)} \approx \exp(- \frac{1}{2} x^TQ(\theta)x + \sum_i \log  (p(y_i|x_i,\theta)))
$$
$$
= (2\pi)^{-n/2} {|P(\theta)|}^{1/2} \exp(- \frac{1}{2} {(x - \mu(\theta))}^T P(\theta) (x - \mu(\theta))), \tag{*}
$$
where...... 
...
4. approximate $p(\theta_j|y)$ by using the points that were already computed during steps 1-3 to construct an interpolant to $\log(\tilde{p}(\theta|y))$ and compute marginals using numerical integration from this $\log(\tilde{p}(\theta|y))$. 

The challenge remains in finding a quick and reliable approach, while keeping the number of evaluation points low. Interested readers are referred to Martins et al.'s (2013, section 3.2) paper for details about the default approach that is used now. 
...
Three approximations are considered: the Gaussian, the Laplace, and the simplified Laplace approximation (Rue et al., 2009, as cited in Morrison, 2017). 

*3.1.2.1 Using Gaussian approximations*

The simplest option is to use the Gaussian approximation 

......

*3.1.2.2 Using Laplace approximations*

The natural way to improve the Gaussian approximation is to use the Laplace approximation

......

*3.1.2.3 Using a simplified Laplace approximation*
...
Interested readers are referred to Lindgren et al.'s (2011) work (*or my dissertation but likely not*) for details about the SPDE approach. 
...
## 5. Spatial Examples Using The `R-INLA` Package
...
### 5.1. A Spatial Areal Example of Leukemia Incident Cases
...
### 5.2. A Spatial Geostatistical Example of Heavy Metal Concentrations 
