...
The latent Gaussian models (LGMs) is a class of three-stage Bayesian hierarchical models. Applications of LGMs include, for example, regression models (e.g. GAMs/GLMs/GAMMs/GLMMs), dynamic models, and spatial (e.g. Gaussian and BYM models) and spatiotemporal models (Rue et.al, 2009). A LGM involves the following stages: 
...
LGMs (latent Gaussian models) is an umbrella class that generalizes the large number of related variants of additive and/or generalized linear models. Consider the Bayesian structured additive model setup, for example, 
$$
...
More specifically, undirected graphs $G$ are typically used to represent the conditional independence properties of the GMRF. An undirected graph $G$ consists of a set of nodes $V$ and edges $E$ 
$$
G = (V, E),
$$ 
...
Although the exact computational cost depends on the actual sparsity pattern in the precision matrix, for the autoregressive example, the $mxm$ sparse precision matrix can be factorized from $\mathcal{O}(m^3)$ to $\mathcal{O}(m)$ with memory requirement reduced from $\mathcal{O}(m^2)$ to $\mathcal{O}(m)$. For models with a spatial structure, the cost is now in $\mathcal{O}(m^{3/2})$ paired with a $\mathcal{O}(m\log(m))$ memory requirement. 

### 2.4. Additive Models and Gaussian Markov Random Fields

The key reason why the INLA approach is so efficient is that it is able to treat the joint distribution as a GMRF with a sparse precision matrix. 


### 2.5. Laplace Approximations 

The Laplace approximation is an old technique for the approximation of integrals (Barndorff-Nielsen & Cox, 1989). The goal is to approximate the integral 
$$
...
$$
$nf(x)$ is the sum of log-likelihoods and $x$ is the unknown parameter. By the central limit theorem, the Gaussian approximation will be exact as $n \to \infty$.

This is because by Taylor series expansion, a function about a point $a$ can be expanded into a sum of terms and a finite number of these terms can be used as an approximation (Morrison, 2017). That is, 
$$
...
$$
where $c$ is some constant. Using a Laplace approximation to approximate the distribution $f(x)$ that is approximately normal, mean $\hat{x}$ can be found by solving $f^{'}(x) = 0$ and variance $\hat{\sigma}^2 = -1 / f^{''}(x)$ at the mode $\hat{x}$. 
...
The main goal of Bayesian inference is to approximate the posterior marginals respectively for the hyperparameters and latent field 
$$
...
$$
expanding the numerator and using a Laplace approximation for the denominator, the above expression becomes 
$$
...
The posterior marginals can be expressed as 
$$
p(x_i | \theta, y) = \int p(x_i|\theta,y) p(\theta|y) d\theta,
$$

resulting in two challenges: 

1. Integrating over $p(\theta|y)$, which is shown to be too computationally costly in Section 3.1.1. 

2. Approximating $p(x_i|\theta,y)$ for a subset of all $i=1,...,n$, where $n$ can be large ($10^3$ to $10^5$). 
...
Plots of predicted values are omitted for now. 
